...
import { SubmitNeighborhood } from "./SubmitNeighborhood";
  ...
  createButton(buttonText: string, icon: JSX.Element = <StarBorder />) {
    return (
      <ListItemButton
        sx={{ pl: 4 }}
        onClick={() => {
          storeDispatch.content.setMainContent(
            <>{`This is where the functionality for ${buttonText} would go.  Unfortunately, this is only a class project, so nothing on the left menu other than "Home" will work`}</>
          );
      ...
      <Collapse in={isExpanded} timeout="auto" unmountOnExit>
        <List component="div" disablePadding>
          ...
          <ListItemButton
            sx={{ pl: 4 }}
            onClick={() => {
              storeDispatch.content.setMainContent(<SubmitNeighborhood/>);
              this.props.updateDrawerState();
            }}
          >
            <ListItemIcon>
